...
export default function Welcome({ auth, products }) {
    ...
    return (
        <>
            ...
            {/* CONTENEDOR PRINCIPAL: overflow-x-hidden evita el scroll horizontal rudo */}
            <div className="min-h-screen overflow-x-hidden bg-[#07111F] text-white relative">

                {/* CONTENEDOR DE FONDOS: Solución para el corte recto de la bola derecha */}
                <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
                    
                    {/* Bola izquierda (Verde) */}
                    <div className="absolute left-[-200px] top-[-200px] h-[500px] w-[500px] rounded-full bg-[#00C896] opacity-20 blur-3xl"></div>
                    
                    {/* SOLUCIÓN AL CORTE RECTO: Posicionamos en right-0 top-0, pero usamos translate-x para sacar 
                        la mitad del círculo hacia afuera de forma fluida. Así el difuminado entra suave a la pantalla */}
                    <div className="absolute top-[-150px] right-0 h-[600px] w-[600px] translate-x-1/3 rounded-full bg-cyan-400 opacity-20 blur-[120px]"></div>
                ...
                <section className="relative grid min-h-screen grid-cols-1 lg:grid-cols-2 z-10">
                    <div className="flex flex-col justify-center px-10 pt-40 lg:px-24">
                        ...
                        <p className="mt-8 max-w-2xl text-xl leading-relaxed text-white/70">
                            ...
                            EcoSwap conecta personas para intercambiar productos sin dinero y reducir residuos.
                        </p>

                        <div className="mt-10 flex flex-wrap gap-5">
                            <Link
                                href={auth.user ? '#' : route('register')}
                ...
                <section className="relative px-6 py-20 lg:px-20 z-10">
                    <div className="mb-16">
                        ...
                        {products && products.map((product) => (
                            ...
                            >
                                <div className="relative overflow-hidden">
                                    <img
                                        src={product.product_images?.[0]?.url || "https://images.unsplash.com/photo-1512496015851-a90fb38ba796"}
                                        alt={product.name}
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="flex items-center gap-5 rounded-[30px] border border-white/5 bg-white/5 p-6">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-xs font-bold uppercase text-white/30">Vendedor</p>
                                            <p className="text-xl font-bold">{selectedProduct.user?.name}</p>
                                        </div>
                                        <button onClick={() => handleChatRedirect(selectedProduct.id)} className="rounded-2xl bg-[#00C896] px-6 py-3 font-black text-black transition-transform hover:scale-105">
                                            Chat
                                        </button>
                                    </div>
                                </div>
