        ...
        <>
            ...
            <div className="min-h-screen overflow-x-hidden bg-[#07111F] text-white relative">
                ...
                <header className="fixed top-0 z-50 w-full border-b border-white/10 bg-white/5 backdrop-blur-xl">
                    <div className="flex items-center justify-between px-10 py-5 lg:px-20">
                        ...
                        <div className="flex items-center gap-4">
                            <Link href="/products/create" className="rounded-2xl bg-gradient-to-r from-[#00C896] to-cyan-400 px-6 py-3 font-bold text-black transition hover:scale-105">
                                + Subir Producto
                            </Link>
                        </div>
                    </div>
                ...
                <main className="relative max-w-7xl mx-auto px-6 pt-40 pb-20 z-10">
                    <div className="mb-12">
                        <h2 className="text-5xl font-black">Mis productos en intercambio</h2>
                        <p className="mt-4 text-xl text-white/60">Aquí puedes revisar y gestionar las cosas que has subido a la plataforma.</p>
                    </div>

                    {products.length === 0 ? (
                        <div className="rounded-[35px] border border-white/10 bg-white/5 p-20 text-center backdrop-blur-xl">
                            <p className="text-xl text-white/50 font-medium">Aún no has añadido ningún producto.</p>
                            <Link href="/products/create" className="mt-6 inline-block text-sm font-bold text-[#00C896] hover:underline">
                        ...
                        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            {products.map((product) => {
                                // Extrae las imágenes soportando snake_case o camelCase dinámicamente
                                const images = product.product_images || product.productImages;
                                
                                // CORRECCIÓN ANTI-BUG: Valida que exista la imagen y que la URL no esté mocha o rota
                                const firstImage = images && images.length > 0 ? images[0].url : null;
                                const hasValidImage = firstImage && firstImage.trim() !== '' && firstImage !== 'storage/' && !firstImage.endsWith('/storage/');

                                return (
                                    <div key={product.id} className="group overflow-hidden rounded-[35px] border border-white/10 bg-white/10 shadow-2xl backdrop-blur-xl p-4 flex flex-col justify-between">
                                        ...
                                        <div className="relative aspect-square w-full rounded-[25px] overflow-hidden bg-black/40 mb-5">
                                            {hasValidImage ? (
                                                <img 
                                                    ...
                                                    onError={(e) => {
                                                        // En caso de que falle de todos modos, oculta la imagen rota y muestra el bloque por defecto
                                            ...
                                            {/* Div por defecto ("Sin foto") que actúa como respaldo si falla el renderizado */}
                                            <div 
                                                className="w-full h-full flex-col gap-2 items-center justify-center text-sm text-white/30"
                                        ...
                                        <div>
                                            <h3 className="truncate text-xl font-bold px-2">{product.name}</h3>
                                            <p className="mt-2 line-clamp-2 text-sm text-white/60 px-2 min-h-[40px]">{product.description}</p>
                                            
                                            <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center px-2 text-xs">
                                                <span className="text-white/50">Cambio por: <strong className="text-white">{product.swap_for || 'Cualquier cosa'}</strong></span>
                                                <span className="font-bold text-cyan-400 text-sm">{product.estimated_value}€</span>
                                            </div>

                                            {/* Acción: Formulario de edición */}
